 ...
 */
  ...
  function (Controller, JSONModel, Filter, FilterOperator, Fragment) {
    "use strict";
    return Controller.extend("znewresource.controller.View2", {
      onInit: function () {
        this.empName = "";
      ...
      onRouteMatched: function (oEvent) {
        ...
        var vLength = sap.ui
          .getCore()
          .getModel("FeederModelForApproveType")
        ...
        for (var i = 0; i < vLength; i++) {
          var empCode = sap.ui
            .getCore()
            .getModel("FeederModelForApproveType")
            .getData().items[i].EmpCode;

          var trackLead = sap.ui
            .getCore()
            .getModel("FeederModelForApproveType")
            .getData().items[i].TrackLead;

          if (selectedEmployeeId === empCode) {
            if (!this.empName) {
              this.empName = sap.ui
                .getCore()
                .getModel("localGlobalModel")
                .getData().items[i].ResourceName;
            }
            var localJSONModel = new sap.ui.model.json.JSONModel(
              sap.ui.getCore().getModel("localGlobalModel").getData().items[i]
            );
            that.getView().setModel(localJSONModel, "localModel");
            for (var j = 0; j < vLength; j++) {
              if (
                this.empName ===
                sap.ui.getCore().getModel("localGlobalModel").getData().items[j]
                  .TrackLead
              ) {
                this._isLead = true;
              }
        ...
        if (this._isLead) {
          ...
          for (
            var k = 0;
            k <
            sap.ui.getCore().getModel("localGlobalModel").getData().items
              .length;
            k++
          ) {
            if (
              this.empName ===
              sap.ui.getCore().getModel("localGlobalModel").getData().items[k]
                .TrackLead
            ) {
        ...
        var _localJSONModelForEmployee = new sap.ui.model.json.JSONModel(
          _localModelForEmployee
        );
        this.getView().setModel(
          _localJSONModelForEmployee,
          "_localJSONModelForEmployee"
        );
        ...
        this.getView()
          .byId("itemCountLabel")
          .setText("No of Reportees:" + aEmployeeList.length);

        var numberofRows = this.getView()
          .byId("tblEmployeeList")
          .getItems().length;
        ...
        that
          .getView()
          .byId("tblEmployeeList")
          .setText("No of Reportees:" + numberofRows);
      },
      ...
      EmpLevel: function (oEvent) {
        ...
        for (
          var i = 0;
          i <
          sap.ui.getCore().getModel("FeederModelForApproveType").getData().items
            .length;
          i++
        ) {
          var user = sap.ui
            .getCore()
            .getModel("FeederModelForApproveType")
            .getData().items[i].Level;

          if (
            topManagementLevels.includes(user.Level) &&
            selectedEmployeeLevel === EmpLevel
          ) {
        ...
        var topManagementResourceNames = topManagementUsers.map(function (
          user
        ) {
        ...
        var usersWithSameResourceName = sap.ui
          .getCore()
          .getModel("FeederModelForApproveType")
          .getData()
          .items.filter(function (user) {
