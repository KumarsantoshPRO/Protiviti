sap.ui.define([
  ...
  "sap/ui/model/json/JSONModel", 
   ...
   "sap/m/MessageBox", 
  "com/zsp/skillportal/model/formatter",
  "sap/m/MessageToast"
],
  /**
   * @param {typeof sap.ui.core.mvc.Controller} Controller
   */
  function (Controller, JSONModel, File, MessageBox, formatter, MessageToast) {
      "use strict";

      return Controller.extend("com.zsp.skillportal.controller.SkillPortal", {
        formatter: formatter,
        onInit: function () {
          //deploytest
        ...
        getDropdownValues: function(sDomValue, sModelName) {
          this.getView().setBusy(true);
            ...
            this.getOwnerComponent().getModel().read("/es_value_helps", {
              filters: [filter],
        ...
        getUserDetails: function() {
          ...
          this.getOwnerComponent().getModel().read("/et_employee_detailsSet", {
            filters: [filter],
            urlParameters: {
            ...
            success: function (oData) {
              ...
              this.getView().setBusy(false);
              if (oData.results && oData.results[0].Status === "N" || oData.results && oData.results[0].Status === undefined || oData.results && oData.results[0].Status === null){
                MessageBox.information("User Details Not available");
            ...
            error: function (oError) {
              this.getView().setBusy(false);
              this.getView().getModel("visibleModel").setProperty("/btnEdit", false);
              MessageBox.information("Please Check the Backend System");
              console.log(oError);
            }.bind(this)
          });
          var status = this.getView().getModel("ViewModel").getProperty("/Status");
          // console.log(status);
        },
        ...
        setEmployeeDetailsMapping: function(oResData) {
            ...
            for (var i = 0; i < oResData.cvdetails.results.length; i++) {
              if (oResData.cvdetails.results[i].CvName.trim() !== "") {
                this.getView().getModel("ResumeModel").setProperty("/" + i + "/fileName", oResData.cvdetails.results[i].CvName);
                this.getView().getModel("ResumeModel").setProperty("/" + i + "/typeName", oResData.cvdetails.results[i].CvType);
                this.getView().getModel("ResumeModel").setProperty("/" + i + "/fileData", oResData.cvdetails.results[i].CvFile);
              }
            }
            ...
            //setting the Marks Scored in Certificate
            
            var certificateModel = this.getView().getModel("CertificateModel");
             var Cresults = oResData.Certificationdet.results;
             Cresults.forEach(function(result) {
             var score = result.Score;
             if (score && score.length > 1 && score.charAt(0) === '0') {
                 result.Score = parseInt(score, 10).toString();
                 }
                   });

                   certificateModel.setData(Cresults);

             //seting the Marks Scored in Education 
        ...
        // Setting the Float Value in the Marks scored Filed as .00 (For Education Details)
        onChangeEducationMrks: function(oEvent){
          var oInput = oEvent.getSource();
            var sValue = oInput.getValue();
            var newValue = sValue.replace(/(\.\d{2})\d+/, "$1");
            if (newValue !== sValue) {                 
              oInput.setValue(newValue);            
             }
        },
        // Setting the Float Value in the Marks scored Field as .00 (For Certification Details)
        onChangeCertficateMrks: function(oEvent){
          var oInput = oEvent.getSource();
          var sValue = oInput.getValue();
          var newValue = sValue.replace(/(\.\d{2})\d+/,"$1");
          if(newValue !== sValue){
      ...
      // },
        ...
        getAttachmentData: function (type, fileName) {
          ...
          this.getOwnerComponent().getModel().read("/es_cvdetails", {
            filters: arrFilter,
            success: function (oData) {
        ...
        _importPhoto: function (file) {
          if (file && window.FileReader) {
            ...
            reader.onload = function (e) {
              const data = e.target.result;
              if (data.trim() !== "") {
                this.getView().getModel("ViewModel").setProperty("/Photo","data:image/png;base64," + btoa(data));
              }
    ...
    educationValidate: function () {
      ...
      if (arrEduData.length > 0) {
        for (var i = 0; i < arrEduData.length; i++) {
          ...
          if (arrEduData[i].MarksScored === "" || arrEduData[i].MarksScored === undefined) {
    ...
    onChangeCountry: function(oEvent) {
      ...
      var sPath = oEvent.getSource().getBindingContext("VisaModel").getPath();
      var iIndex = parseInt(sPath.split("/")[1]);
      this.getView().getModel("VisaModel").setProperty("/" + iIndex + "/Country", oSelectedItem)
    },
    ...
    callSave: function() {
        ...
        var arrCertiData = [], arrProjectData = [], arrEducData = [], arrVisaData = [], 
        arrResData = [], arrSSkill = [], arrIndExp = [];  
        ...
        for (var i = 0; i < oEducationalModelData.length; i++) {
          var oEduObject = {
            ...
            "Board" : "",
            "MarksScored" : oEducationalModelData[i].MarksScored.toString(),
        ...
        for (var i = 0; i < oProjectModelData.length; i++) {
          var oProjObject = {
            ...
            // "ProjectType" : "01",  // need to check
            "LegacySystem" : oProjectModelData[i].LegacySystem, 
        ...
        //return;
        this.getView().getModel().create('/et_employee_detailsSet', oEntryData,{
    ...
    onEdit: function () {
      // var user = this.getView().getModel("ViewModel").getProperty("/Pernr")
      // console.log(user)
      // var oVisibleModel =
